<template>
    <header class="z-10 py-4 bg-white shadow-md dark:bg-gray-800">
        <div class="container flex items-center justify-between h-full px-6 mx-auto text-purple-600 dark:text-purple-300">
            <!-- Mobile hamburger -->
            <button class="p-1 -ml-1 mr-5 rounded-md md:hidden focus:outline-none focus:shadow-outline-purple" @click="$emit('side-menu-was-toggled')" aria-label="Menu">
            ...
            <ul class="flex items-center flex-shrink-0 space-x-6">
                <!-- Theme toggler -->
                <li class="flex">
                    <button class="rounded-md focus:outline-none focus:shadow-outline-purple" @click="$emit('theme-was-toggled')" aria-label="Toggle color mode">
                        <span v-show="!dark">
                ...
                <li class="relative">
                    ...
                    <transition leave-active-class="transition-opacity duration-200" leave-to-class="opacity-0">
                        <template v-if="isNotificationsMenuOpen">
                            <ul v-click-outside="closeNotificationsMenu" class="absolute right-0 w-56 p-2 mt-2 space-y-2 text-gray-600 bg-white border border-gray-100 rounded-md shadow-md dark:text-gray-300 dark:border-gray-700 dark:bg-gray-700" aria-label="submenu">
                                <li class="flex">
                ...
                <li class="relative">
                    ...
                    <transition leave-active-class="transition-opacity duration-200" leave-to-class="opacity-0">
                        <template v-if="isProfileMenuOpen">
                            <ul @click-outside="closeProfileMenu" @keydown.escape="closeProfileMenu" class="absolute right-0 w-56 p-2 mt-2 space-y-2 text-gray-600 bg-white border border-gray-100 rounded-md shadow-md dark:border-gray-700 dark:text-gray-300 dark:bg-gray-700" aria-label="submenu">
                                <li class="flex">
                                    <inertia-link :href="route('profile.show')" class="inline-flex items-center w-full px-2 py-1 text-sm font-semibold transition-colors duration-150 rounded-md hover:bg-gray-100 hover:text-gray-800 dark:hover:bg-gray-800 dark:hover:text-gray-200">
                                ...
                                <li class="flex">
                                    <form id="formLogout" @submit.prevent="submit" class="inline-flex items-center w-full">
                                        <button form="formLogout" type="submit" class="focus:outline-none inline-flex items-center w-full px-2 py-1 text-sm font-semibold transition-colors duration-150 rounded-md hover:bg-gray-100 hover:text-gray-800 dark:hover:bg-gray-800 dark:hover:text-gray-200">
                                            <svg class="w-4 h-4 mr-3" aria-hidden="true" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" stroke="currentColor">
...
export default {
    ...
    emits: ['side-menu-was-toggled', 'theme-was-toggled'],
